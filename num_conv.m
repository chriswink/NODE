...
for i=1:1:9%Anzahl Zeitschritte
	m = 2^i;
...
printf('\n Plot: Anz. Zeitschritte: %d',length(L_Ei.grid));
plot(L_Ei.grid,(L_Ei.x),'r');
plot(L_Mi.grid,(L_Mi.x),'g');
plot(L_Ee.grid,(L_Ee.x),'y');
plot(L_RK.grid,(L_RK.x),'c');
